...
void LinkedList<T>::insertOrdered(const T &newData)
{
  ...
  {
    Node *current = head_;
    while (nullptr != current->next && current->next->data < newData)
    {
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T> &other) const
{
  ...
  Node *lCursor = left.head_;
  Node *rCursor = right.head_;

  while (nullptr != lCursor && nullptr != rCursor)
  {
    if (lCursor->data < rCursor->data)
    {
      merged.pushBack(lCursor->data);
      lCursor = lCursor->next;
    }
    else
    {
      merged.pushBack(rCursor->data);
      rCursor = rCursor->next;
    }
  }

  while (nullptr != lCursor)
  {
    merged.pushBack(lCursor->data);
    lCursor = lCursor->next;
  }

  while (nullptr != rCursor)
  {
    merged.pushBack(rCursor->data);
    rCursor = rCursor->next;
  }
