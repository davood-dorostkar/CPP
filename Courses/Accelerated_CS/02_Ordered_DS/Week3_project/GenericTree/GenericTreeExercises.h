...
static void treeFactory(GenericTree<int> &tree)
{
  ...
  // ...
  GenericTree<int>::TreeNode *root = tree.getRootPtr();
  // if(!root->childrenPtrs.empty())
  root->childrenPtrs.clear();
  root->data = 4;
  GenericTree<int>::TreeNode *child8 = root->addChild(8);
  GenericTree<int>::TreeNode *child15 = root->addChild(15);
  GenericTree<int>::TreeNode *child16 = child8->addChild(16);
  GenericTree<int>::TreeNode *child23 = child8->addChild(23);
  GenericTree<int>::TreeNode *child42 = child16->addChild(42);
}
...
std::vector<T> traverseLevels(GenericTree<T> &tree)
{
  ...
  while (!q.empty())
  {
    for (TreeNode *i : q.front()->childrenPtrs)
    {
      q.push(i);
    }
